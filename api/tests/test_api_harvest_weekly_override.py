...
def test_weekly_override_period_is_determined_by_date_param(admin_client, device_client, user_client):
    ...
    resp = admin_client.patch(
        f"/harvest/amount/weekly/size/S?date={d.isoformat()}",
        {"count": 99},
        format="json",
    )
    ...
    ov = HarvestAggregateOverride.objects.get(period_type="weekly", size_id="S", period=expected_period)
    assert ov.total_count == 99

    # And: weekly-by-category API reflects the override for that period
    resp2 = user_client.get("/harvest/amount/weekly/size/S?page=1&page_size=50")
