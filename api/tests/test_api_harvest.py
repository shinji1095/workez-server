...
def test_create_harvest_amount_add_success(device_client, admin_client):
    ...
    for k in ("id", "lot_name", "size_id", "rank_id", "count", "occurred_at", "created_at"):
        assert k in data
    assert data["lot_name"] == lot_name
    assert data["size_id"] == size_id
    assert data["rank_id"] == rank_id
    assert data["count"] == 3
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (lambda p: p.pop("lot_name"), 400),
        (lambda p: p.pop("size_id"), 400),
        (lambda p: p.pop("rank_id"), 400),
...
def test_list_harvest_amount_daily_success_contains_aggregate(device_client, user_client, admin_client):
    ...
    # At least one aggregated row should reflect what we added (period + total_count)
    found = False
    for item in data["items"]:
        if item.get("period") == target_period and item.get("total_count") == 3:
            found = True
            break
...
# -----------------------------------------------------------------------------
# GET /harvest/amount/daily/size/{sizeId} (user/admin; device forbidden)
# -----------------------------------------------------------------------------
def test_retrieve_harvest_amount_daily_size_success(device_client, user_client):
    lot_name = "1e"
    size_id = "S"
    rank_id = "A"
    occurred_at = "2025-01-15T09:00:00+09:00"
    ...
    resp = user_client.get(f"/harvest/amount/daily/size/{size_id}?page=1&page_size=50")
    assert resp.status_code == 200, _payload(resp)
    ...
    assert any(
        item.get("period") == target_period and item.get("size_id") == size_id and item.get("total_count") == 3
        for item in data["items"]
    ), data["items"]


def test_retrieve_harvest_amount_daily_size_not_found(user_client):
    resp = user_client.get("/harvest/amount/daily/size/NO_SUCH_SIZE?page=1&page_size=10")
    assert resp.status_code == 404, _payload(resp)
    _assert_error_envelope(resp)


def test_retrieve_harvest_amount_daily_size_requires_auth_and_permission(device_client, user_client):
    anon = _anon_client()

    r0 = anon.get("/harvest/amount/daily/size/S?page=1&page_size=10")
    assert r0.status_code == 401, _payload(r0)
    _assert_error_envelope(r0)

    r1 = device_client.get("/harvest/amount/daily/size/S?page=1&page_size=10")
    assert r1.status_code == 403, _payload(r1)
    _assert_error_envelope(r1)

    r2 = user_client.get("/harvest/amount/daily/size/S?page=1&page_size=10")
    assert r2.status_code == 200, _payload(r2)
...
def test_retrieve_harvest_amount_daily_size_pagination_boundaries(user_client, qs):
    resp = user_client.get(f"/harvest/amount/daily/size/S?{qs}")
    assert resp.status_code == 400, _payload(resp)
    _assert_error_envelope(resp)
