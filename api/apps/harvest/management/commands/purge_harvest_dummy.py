...
class Command(BaseCommand):
    help = "Delete dummy harvest records by event_id prefix."
    ...
    def handle(self, *args, **options) -> None:
        ...
        qs = HarvestRecord.objects.filter(event_id__gte=start_uuid, event_id__lte=end_uuid)
        count = qs.count()

        if dry_run:
            self.stdout.write(
                f"[dry-run] would delete {count} harvest records with event_id_prefix={event_id_prefix}"
            )
            return

        deleted, _ = qs.delete()
        self.stdout.write(
            self.style.SUCCESS(f"Deleted {deleted} harvest records with event_id_prefix={event_id_prefix}")
        )
